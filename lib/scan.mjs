...
export function filterLiveSessions(sessionFiles, liveClaudePids) {
  // liveClaudePids may be a Set (liveness only) or a Map pid -> child count.
  const kidsOf =
    typeof liveClaudePids.get === "function" ? (pid) => liveClaudePids.get(pid) || 0 : () => 0;
  const live = [];
  for (const s of sessionFiles) {
    ...
    live.push({
      ...
      // A live child process ⇒ an attached shell/monitor is still working.
      bg: kidsOf(s.pid) > 0,
