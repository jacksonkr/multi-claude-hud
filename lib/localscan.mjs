...
function winProcs(timeoutMs) {
  // Count child processes, but exclude MCP servers (always-present
  // infrastructure Claude spawns for configured MCP) so they don't look like a
  // background task. Their command line contains "mcp".
  const ps =
    ...
    "[pscustomobject]@{ pid=$c; kids=@($all | Where-Object { $_.ParentProcessId -eq $c -and $_.CommandLine -notmatch 'mcp' }).Count } } | ConvertTo-Json -Compress";
    ...
    execFile(
      ...
      (err, stdout) => {
        ...
        for (const line of stdout.split("\n")) {
          ...
          rows.push({ pid, comm: m[3] });
          // Exclude MCP servers from the child count (always-present infra).
          if (!/mcp/i.test(m[4])) childCount.set(ppid, (childCount.get(ppid) || 0) + 1);
        }
        for (const r of rows) {
