...
// Claude's session status → HUD status:
//   busy → working (green), waiting → waiting (yellow, needs your answer),
//   idle → idle (red, finished) UNLESS an attached shell is still working
//          (bg) → monitoring (green/yellow split).
export function statusOf(raw, bg) {
  if (raw === "busy") return "working";
  if (raw === "waiting") return "waiting";
  if (bg) return "monitoring";
  return "idle";
}
...
const ACTIVITY = {
  ...
  monitoring: "background task running",
  idle: "idle",
};

export function toRecord(s, ctx, now) {
  const status = statusOf(s.status, s.bg);
  const cwd = s.cwd || "";
  return {
    id: s.sessionId,
...
const sig = (r) => `${r.status}|${r.lastWorkingAt}|${r.name}|${r.project}`;
